...
export default function VehiclePage(){
    ...
    const {runContractFunction: getVehicle, isLoading, isFetching} = useWeb3Contract({
        abi: abi,
                    ...
                    {contractAddress ? (
                        <div>
                            {isLoading || isFetching ? (
                                ...
                                <div>
                                    <h1 className="text-3xl font-bold">LICENSE PLATE: {vehicle?.licensePlate}</h1>
                                    <h2 className="text-2xl">MILEAGE: {vehicle?.mileage.toString()}</h2>
                                    <h2 className="text-2xl">OWNER: {vehicle?.owner.toString()}</h2>
                                    ...
                                    {mileageRecords?.map((mileageRecord: MileageRecord) => {
                                        return(
                                            <div key={mileageRecord.timestamp.toString()}>
                                                <h2 className="text-2xl">MILEAGE: {mileageRecord.mileage.toString()}</h2>
                                                <h2 className="text-2xl">TIMESTAMP: {mileageRecord.timestamp.toString()}</h2>
                                            </div>
                                        );
                                    })}
                                </div>
